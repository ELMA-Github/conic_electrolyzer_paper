...
################# Model for conic hydrogen production (no left part of the curve) #################
def model_HYP_SOC(config_dict,El,St,Wind,Demand,Prices):
    ...
    m.xi_RT = pyo.Var(m.TUO,bounds = (0, None)) # Auxiliary variable for each conic region
    ...
    def cst_h_prod_RT(m, t, u,o):
          return m.h_el_u_RT[t,u,o] == El['D_0']*m.z_on_RT[t,u,o]+El['D_1']*m.p_s_RT[t,u,o]+El['D_2']*m.xi_RT[t,u,o]
    m.cst_h_prod_RT = pyo.Constraint(m.TUO, rule = cst_h_prod_RT)
    
    def cst_h_prod_q_RT(m,t,u,o):
        return m.xi_RT[t,u,o]>=m.p_s_RT[t,u,o]**2
    m.cst_h_prod_q_RT = pyo.Constraint(m.TUO, rule = cst_h_prod_q_RT)
    ...
    xi_RT = np.reshape(np.array([m.xi_RT[t,u,o].value for t in m.T for u in m.U for o in m.O]), (Prices['N_t'],El['N_u'],Wind['N_omega']))
