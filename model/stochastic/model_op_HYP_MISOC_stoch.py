...
################# Model for conic hydrogen production on two segments#################
def model_HYP_MISOC(config_dict,El,St,Wind,Demand,Prices):
    ...
    m.z_s = pyo.Var(m.TUS, domain=pyo.Binary) # binary variable for segment s, unit u , time t
    m.p_s = pyo.Var(m.TUS, bounds = (0, None)) # power for segment s, unit u , time t
    
    m.xi = pyo.Var(m.TUS,bounds = (0, None)) # Auxiliary variable for each conic region
    ...
    m.z_s_RT = pyo.Var(m.TUSO, domain=pyo.Binary) # binary variable for segment s, unit u , time t
    m.p_s_RT = pyo.Var(m.TUSO, bounds = (0, None)) # power for segment s, unit u , time t    
    
    m.xi_RT = pyo.Var(m.TUSO,bounds = (0, None)) # Auxiliary variable for each conic region
    ...
    # Segments
    def cst_z_reg(m,t,u):
        return sum(m.z_s[t,u,s] for s in m.S) == m.z_on[t,u]
    m.cst_z_reg = pyo.Constraint(m.TU, rule = cst_z_reg)
    
    def cst_p_u_tot(m,t,u):
        return m.p_el_u[t,u]==sum(m.p_s[t,u,s] for s in m.S)+El['P_sb']*El['El_cap']*m.z_sb[t,u]
    m.cst_p_u_tot = pyo.Constraint(m.TU, rule = cst_p_u_tot)
    
    def cst_ps_lb(m,t,u,s):
        return m.p_s[t,u,s]>=m.z_s[t,u,s]*El['p_val'][s]
    m.cst_ps_lb = pyo.Constraint(m.TUS, rule = cst_ps_lb)
    
    def cst_ps_ub(m,t,u,s):
        return m.p_s[t,u,s]<=m.z_s[t,u,s]*El['p_val'][s+1]
    m.cst_ps_ub= pyo.Constraint(m.TUS, rule = cst_ps_ub)
    
    # Hydrogen production
    def cst_h_prod(m, t, u):
        return m.h_el_u[t,u] == El['D_0']*m.z_s[t,u,0]+El['D_1']*m.p_s[t,u,0]+El['D_2']*m.xi[t,u,0] + El['E_0']*m.z_s[t,u,1]+El['E_1']*m.p_s[t,u,1]+El['E_2']*m.xi[t,u,1]
    m.cst_h_prod = pyo.Constraint(m.TU, rule = cst_h_prod)

    def cst_h_prod_q(m,t,u,s):
         return m.xi[t,u,s]>=m.p_s[t,u,s]**2
    m.cst_h_prod_q = pyo.Constraint(m.TUS, rule = cst_h_prod_q)
    ...
    # Segments
    def cst_z_reg_RT(m,t,u,o):
        return sum(m.z_s_RT[t,u,s,o] for s in m.S) == m.z_on_RT[t,u,o]
    m.cst_z_reg_RT = pyo.Constraint(m.TUO, rule = cst_z_reg_RT)
    
    def cst_p_u_tot_RT(m,t,u,o):
        return m.p_el_u_RT[t,u,o]==sum(m.p_s_RT[t,u,s,o] for s in m.S)+El['P_sb']*El['El_cap']*m.z_sb_RT[t,u,o]
    m.cst_p_u_tot_RT = pyo.Constraint(m.TUO, rule = cst_p_u_tot_RT)
    
    def cst_ps_lb_RT(m,t,u,s,o):
        return m.p_s_RT[t,u,s,o]>=m.z_s_RT[t,u,s,o]*El['p_val'][s]
    m.cst_ps_lb_RT = pyo.Constraint(m.TUSO, rule = cst_ps_lb_RT)
    
    def cst_ps_ub_RT(m,t,u,s,o):
        return m.p_s_RT[t,u,s,o]<=m.z_s_RT[t,u,s,o]*El['p_val'][s+1]
    m.cst_ps_ub_RT= pyo.Constraint(m.TUSO, rule = cst_ps_ub_RT)
    
    # Hydrogen production
    def cst_h_prod_RT(m, t, u, o):
        return m.h_el_u_RT[t,u,o] == El['D_0']*m.z_s_RT[t,u,0,o]+El['D_1']*m.p_s_RT[t,u,0,o]+El['D_2']*m.xi_RT[t,u,0,o] + El['E_0']*m.z_s_RT[t,u,1,o]+El['E_1']*m.p_s_RT[t,u,1,o]+El['E_2']*m.xi_RT[t,u,1,o]
    m.cst_h_prod_RT = pyo.Constraint(m.TUO, rule = cst_h_prod_RT)

    def cst_h_prod_q_RT(m,t,u,s,o):
         return m.xi_RT[t,u,s,o]>=m.p_s_RT[t,u,s,o]**2
    m.cst_h_prod_q_RT = pyo.Constraint(m.TUSO, rule = cst_h_prod_q_RT)
    ...
    z_s = np.reshape(np.array([m.z_s[t,u,s].value for t in m.T for u in m.U for s in m.S]), (Prices['N_t'],El['N_u'],El['N_s']))
    p_s = np.reshape(np.array([m.p_s[t,u,s].value for t in m.T for u in m.U for s in m.S]), (Prices['N_t'],El['N_u'],El['N_s']))   
    xi = np.reshape(np.array([m.xi[t,u,s].value for t in m.T for u in m.U for s in m.S]), (Prices['N_t'],El['N_u'],El['N_s']))
    ...
    z_s_RT = np.reshape(np.array([m.z_s_RT[t,u,s,o].value for t in m.T for u in m.U for s in m.S for o in m.O]), (Prices['N_t'],El['N_u'],El['N_s'],Wind['N_omega']))
    p_s_RT = np.reshape(np.array([m.p_s_RT[t,u,s,o].value for t in m.T for u in m.U for s in m.S for o in m.O]), (Prices['N_t'],El['N_u'],El['N_s'],Wind['N_omega'])) 
    
    xi_RT = np.reshape(np.array([m.xi_RT[t,u,s,o].value for t in m.T for u in m.U for s in m.S for o in m.O]), (Prices['N_t'],El['N_u'],El['N_s'],Wind['N_omega'])) 
