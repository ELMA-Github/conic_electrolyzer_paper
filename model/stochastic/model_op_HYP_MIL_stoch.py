...
def model_HYP_MIL(config_dict,El,St,Wind,Demand,Prices):
    ...
    def cst_h_prod(m, t, u):
        return m.h_el_u[t,u] == sum(m.p_s[t,u,s]*El['a'][s]+m.z_s[t,u,s]*El['b'][s] for s in m.S)
    ...
    def cst_h_prod_RT(m, t, u, o):
        return m.h_el_u_RT[t,u, o] == sum(m.p_s_RT[t,u,s,o]*El['a'][s]+m.z_s_RT[t,u,s,o]*El['b'][s] for s in m.S)
